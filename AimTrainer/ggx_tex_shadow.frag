/*
ggx_tex_shadow.frag: fragment shader for GGX illumination model, with shadow rendering using shadow map
...
// the execution time of the application is passed to the shader using an uniform variable (used to animate the procedural texture)
uniform float timer;

// flag that activates the normal maps
uniform bool normalMapping;

// flag that activates the procedural texture
uniform bool procedural;

// value needed to activate target specific behaviour
...
// GGX illumination method
vec3 GGX(vec4 surfaceColor, vec3 Ntexture) 
{
    // normalized per-fragment normal
    vec3 N = Ntexture;
...
// set of functions used to create noise in the snoise function 
...
// function used to create the noise needed in the procedural texture
vec4 noise() {
...
{   
    ...
    // we assign the surface color of the object (special case if we need to use the procedural texture)
    if(procedural){
        surfaceColor = noise();
    }
    else{
    ...
    // normal sampling form the normal map and normalization of the normals
    ...
    // shadow value = 1 -> fragment is in shadow
    // shadow value = 0 -> fragment is in light
    // we use (1-shadow) as weight to apply to the illumination model
    // we use shadow*0.7 so that the fragments in shadow are not completely black and the shadows look smoother wrt the fragments in light
    vec3 finalColor = (1.0 - (shadow*0.7)) * (lambert + specular) * NdotL;

    // if the fragments are not hit by the light we still give them some color (only based on the lambertian component) to make dark areas more belivable and with less contrast wrt the rest of the scene 
    if(NdotL==0.0){
        finalColor = 0.1*(lambert);
    }

    
    // special behaviour is the fragments are the ones of a target object 
    // we consider targets as they are always in light to make them more visible to the player 
    if(isTarget){
        finalColor = (lambert + specular) * NdotL;
    }

    // we add the GGX component calculated based on the pointLight attributes to create reflections based on the single point light we can have in the scene
    // the color also depends on the color of the pointLight
